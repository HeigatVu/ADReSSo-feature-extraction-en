from traditionalApproach import featureSelection
...
def scale_and_select(X_train: pd.DataFrame,
    ...
    if strategy == "pca":
        ...
        X_train_sel = pd.DataFrame(selector.transform(X_train_scaled), columns=cols_train, index=X_train.index)
        X_test_sel = pd.DataFrame(selector.transform(X_test_scaled), columns=cols_train, index=X_test.index)
    else:
...
def fused_feature(X_ling_df_train:pd.DataFrame,
    ...
    if len(X_ling_df_train) == len(X_acoustic_df_train):
        # Linguisti feature
        X_ling, y_ling = X_ling_df_train.drop(columns=["label"]), X_ling_df_train["label"]
        ...
        # Acoustic feature
        X_ac, y_ac = X_acoustic_df_train.drop(columns=["label"]), X_acoustic_df_train["label"]
        ...
        if (X_compare_df_train is not None) and \
            ...
            X_compare, y_compare = X_compare_df_train.drop(columns=["label"]), X_compare_df_train["label"]
